...
export default function CarryForwardHistoryPage() {
  ...
  const fetchHistory = async () => {
    ...
    setError(null);
    try {
      const res = await authenticatedFetch('http://localhost:3000/leaves/carryforward/history');
      if (!res.ok) throw new Error('Failed to fetch carry-forward history');
  ...
  return (
    <DashboardLayout title="Carry-Forward History" description="Recent automatic carry-forward transactions">
      <div className="space-y-6">
        {loading ? (
          <div className="bg-[#2a2a2a] rounded-lg p-8 text-center">
            <Loader className="animate-spin mx-auto text-blue-500 mb-4" size={36} />
            <p className="text-gray-400">Loading carry-forward history...</p>
          </div>
        ) : (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            {error && <div className="mb-4 text-red-400">{error}</div>}
            {records.length === 0 ? (
              <div className="p-8 text-center text-gray-400">No carry-forward records found.</div>
            ) : (
